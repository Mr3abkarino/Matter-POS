...
/* ---------------- 📌 VERSION CONTROL (v11.0.0 STABLE MASTER) ---------------- */
...
export default function SmartPOSApp() {
  useEffect(() => {
    ...
    if (savedVersion !== APP_VERSION) {
      ...
      localStorage.setItem("pos_products_v11", JSON.stringify(DEFAULT_PRODUCTS));
      localStorage.setItem("pos_delivery_zones_v11", JSON.stringify(DEFAULT_DELIVERY_ZONES));
      localStorage.setItem("pos_categories_v11", JSON.stringify(DEFAULT_CATEGORIES));
      localStorage.setItem("pos_customers_v11", JSON.stringify(DEFAULT_CUSTOMERS));
    }
  }, []);
  ...
  const [categories, setCategories] = useState(() => JSON.parse(localStorage.getItem("pos_categories_v11") || JSON.stringify(DEFAULT_CATEGORIES)));
  const [products, setProducts] = useState(() => JSON.parse(localStorage.getItem("pos_products_v11") || JSON.stringify(DEFAULT_PRODUCTS)));
  const [completedOrders, setCompletedOrders] = useState(() => JSON.parse(localStorage.getItem("pos_orders_v11") || "[]"));
  const [deliveryZones, setDeliveryZones] = useState(() => JSON.parse(localStorage.getItem("pos_delivery_zones_v11") || JSON.stringify(DEFAULT_DELIVERY_ZONES)));
  const [customers, setCustomers] = useState(() => JSON.parse(localStorage.getItem("pos_customers_v11") || JSON.stringify(DEFAULT_CUSTOMERS)));
  ...
  useEffect(() => { localStorage.setItem("pos_categories_v11", JSON.stringify(categories)); }, [categories]);
  useEffect(() => { localStorage.setItem("pos_products_v11", JSON.stringify(products)); }, [products]);
  useEffect(() => { localStorage.setItem("pos_orders_v11", JSON.stringify(completedOrders)); }, [completedOrders]);
  useEffect(() => { localStorage.setItem("pos_delivery_zones_v11", JSON.stringify(deliveryZones)); }, [deliveryZones]);
  useEffect(() => { localStorage.setItem("pos_customers_v11", JSON.stringify(customers)); }, [customers]);
      ...
      <div dir="rtl" className="h-screen w-full bg-slate-900 flex items-center justify-center p-4 font-sans select-none">
        <div className="bg-white rounded-3xl max-w-sm w-full p-8 space-y-6 shadow-2xl">
          <div className="text-center space-y-2">
            ...
            <p className="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2.5 py-0.5 rounded-full inline-block">Master Unlocked v{APP_VERSION}</p>
          </div>
  ...
  return (
    <div dir="rtl" className="h-screen w-full bg-slate-50 flex font-sans select-none overflow-hidden text-slate-800 relative">
      ...
      {/* 📌 SIDEBAR - FULL UNLOCKED NAVIGATION */}
      <aside className={`
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden pt-14 lg:pt-0 relative">
        ...
        {/* ⚙️ FULL SETTINGS VIEW - UNABRIDGED */}
